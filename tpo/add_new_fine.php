...
$query= "SELECT * FROM fine_tickets";
	...
	while ( $result = mysqli_fetch_assoc($result_set) ) {
		$province_list .= "<option value=\"{$result['fine_id']}\">{$result['provision']}</option>";
	}
?>
<!----------------------------------------------Provision dropdown goes here ---------------------------------------------------------->
<?php
...
//---------------------------------------issue fine code goes here ---------------------------------------------------------------
	...
	if (empty($license)) {
    ...
    else
     {
        ...
        include "../connection.php";
		$sql2 = "INSERT INTO issued_fines(police_id, license_id, vehicle_no, class_of_vehicle, place, issued_date, issued_time, expire_date, court, court_date, provisions, total_amount, status)VALUES('$policeid', '$license', '$vehicleno', '$classofvehicle', '$place', '$issuedate', '$issuetime', '$expiredate', '$court', '$courtdate', '$provisions', '$fineamount', 'pending')";
...
<head>
    <title>Add Violation | Traffic Police Officer</title>
        ...
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="container-fluid">
                ...
                <div class="card mb-4">
                    ...
                    <button type="submit" class="btn btn-primary mb-2" name="search"><i class="fas fa-search"></i> Add Driver Details</button>
                ...
                <div class="form-row">
                    <div class="form-group col-md-6">
                        ...
                        <input type="date" class="form-control" id="expire_date" value="<?php echo date('Y-m-d', strtotime("+14 days")); ?>" name="expiredated" placeholder="Expire Date" disabled>
                        <input type="hidden" class="form-control" id="expire_date" value="<?php echo date('Y-m-d', strtotime("+14 days")); ?>" name="expiredate" placeholder="Expire Date">
                </div>
                <div class="form-group col-md-6">
                        <label for="court_date">Court Date</label>
                        <input type="date" class="form-control" id="court_date" value="<?php echo date('Y-m-d', strtotime("+15 days")); ?>" name="courtdated" placeholder="Court Date" disabled>
                        <input type="hidden" class="form-control" id="court_date" value="<?php echo date('Y-m-d', strtotime("+15 days")); ?>" name="courtdate" placeholder="Court Date">
                </div>
            </div>
            ...
            <div class="form-row">
                ...
                <button type="submit" class="btn btn-primary" name="issuefine" data-toggle="tooltip" title="Click to issue the fine"><i class="fas fa-plus-circle"></i> Issue Fine</button>
            </form>
            </div>
        </div>
    </div>
