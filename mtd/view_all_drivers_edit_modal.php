...
if (!empty($_POST)) {
    ...
    if (!empty($license_id)) {
        ...
        if (mysqli_query($conn, $query)) {
          header("Location: ../mtd/view_all_drivers.php?success=Revenue License updated successfully!");
        exit();
        } else {
          header("Location: ../mtd/view_all_drivers.php?error=Failed to Update Road Tax!");
          exit();
        }
    } else {
     header("Location: ../mtd/view_all_drivers.php?error=Missing required fields!");
     exit();
    }
